...
Veasy = (function() {
  ...
  Veasy.prototype.drawStack = function(series, opt) {
    var allXrange, allYrange, area, category10, color, err, l, mergedSeries, rangeInEachX, stack, x, xAxis, xScale, xType, xaxis, y, yAxis, yScale, yType, yaxis, _ref, _ref1, _ref2, _ref3,
      _this = this;
    if (opt == null) {
      opt = {};
    }
  ...
  Veasy.prototype.drawScatterMatrix = function(serie, opt) {
    var attrs, color, data, extent, height, idx1, idx2, k, k1, k2, margin, matrix, padding, r, scale, scaleRange, scales1, scales2, text, type1, type2, width, xAxis, xaxis, yAxis, yaxis, _i, _len, _results;
    if (opt == null) {
      opt = {};
    }
    opt = new Option(this.opt, opt);
    ...
    for (idx1 = _i = 0, _len = attrs.length; _i < _len; idx1 = ++_i) {
      ...
      _results.push((function() {
        ...
        _results1 = [];
        for (idx2 = _j = 0, _len1 = attrs.length; _j < _len1; idx2 = ++_j) {
          k2 = attrs[idx2];
          ...
          scales1 = (type1 === 'Date' ? d3.time.scale() : d3.scale.linear()).range([width, 0]).domain(d3.extent(serie.data, function(d) {
            return d[k1];
          }));
          scales2 = (type2 === 'Date' ? d3.time.scale() : d3.scale.linear()).range([0, height]).domain(d3.extent(serie.data, function(d) {
            return d[k2];
          }));
          if (idx2 === 0) {
            text = matrix.append('text').text(attrs[idx1]);
            text.attr('transform', "rotate(-90) translate(-" + height + ", -40)");
            yaxis = d3.svg.axis().scale(scales1).orient('left').ticks(5);
            if (type1 === 'Date') {
          ...
          if (idx1 === 0) {
            ...
            xaxis = d3.svg.axis().scale(scales2).orient('top').ticks(5);
            if (type2 === 'Date') {
          ...
          if (idx1 === idx2) {
            ...
            extent = d3.extent(serie.data, function(d) {
              return d[k1];
            });
            scaleRange = d3.scale.linear().domain(extent).range([0, 9]);
            data = d3.nest().key(function(d) {
              return 0 | scaleRange(d[k1]);
            ...
            matrix.append('g').selectAll('rect').data(data).enter().append('rect').attr('width', (width + r * 2) / data.length).attr('height', function(d) {
              return height - scale(d.values);
            }).attr('x', function(d, idx) {
              return idx * (width + r * 2) / data.length - r;
            }).attr('y', function(d) {
              return scale(d.values) + r;
            }).attr('fill', color);
            continue;
          }
          _results1.push(matrix.append('g').selectAll('circle.scatter').data(serie.data).enter().append('circle').attr('class', 'scatter').attr('cy', function(d) {
            return scales1(d[k1]);
          }).attr('cx', function(d) {
            return scales2(d[k2]);
          }).attr('r', 4).attr('fill', 'none').attr('stroke', 'black'));
        }
        return _results1;
      }).call(this));
    }
