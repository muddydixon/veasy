...
Veasy = (function() {
  ...
  Veasy.prototype.legend = function(args) {
    if (args == null) {
      args = {};
    }
    if (!((args.position != null) || (args.flow != null))) {
      return this;
    }
    this._legend = {};
    if (args.position != null) {
      this._legend.position = args.position;
    }
    if (args.flow != null) {
      this._legend.flow = args.flow;
    }
    return this;
  };

  Veasy.prototype.appendLegend = function(series, color) {
    var $list, category10, cols, flow, height, left, legend, list, padding, position, rect, rows, theight, top, twidth, width,
      _this = this;
    ...
    padding = theight * 0.2;
    flow = this._legend.flow;
    if ((flow != null) && typeof flow === 'string' && flow.match('v')) {
    ...
    } else if ((flow != null) && typeof flow === 'string' && (rows = flow.match(/(\d+)x/))) {
      rows = rows[1];
      list.attr('dy', function(d, idx) {
    ...
    } else if ((flow != null) && typeof flow === 'string' && (cols = flow.match(/x(\d+)/))) {
      cols = cols[1];
      list.attr('dy', function(d, idx) {
    ...
    position = this._legend.position;
    if ((position != null) && typeof position === 'number') {
      top = position;
      left = position;
    } else if ((position != null) && typeof position === 'string') {
      if (position.match('n')) {
        top = 20;
      }
      if (position.match('w')) {
    ...
    } else if ((position != null) && position instanceof Array) {
      if (position.length === 1) {
        ...
        top = this.height - height - position[2];
        left = position[1];
      } else {
        top = this.height - height - position[2];
        left = this.width - width - position[3];
      }
    }
