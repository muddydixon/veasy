...
Veasy = (function() {
  ...
  Veasy.prototype.drawLine = function(series, opt) {
    ...
    series.forEach(function(serie, sid) {
      ...
      return dot = _this.svg.selectAll("circle.serie-" + sid).data(serie.data).enter().append('circle').attr('class', "serie-" + sid).attr('cx', function(d) {
        return x(_this._x(d));
      }).attr('cy', function(d) {
        return y(_this._y(d));
      }).attr('r', 5).attr('fill', color).attr('stroke', 'none').attr('stroke-width', 3).style('cursor', 'pointer').on('mouseover', function(d) {
        var dom;
