...
Veasy = (function() {
  ...
  Veasy.prototype.drawLine = function(series, opt) {
    var allXrange, allYrange, dot, idx, l, line, mergedSeries, serie, sort, tooltipFormat, x, xAxis, xScale, xType, xaxis, y, yAxis, yScale, yType, yaxis, _i, _j, _len, _len1, _ref, _ref1,
      _this = this;
    if (opt == null) {
      opt = {};
    }
    ...
    for (idx = _j = 0, _len1 = series.length; _j < _len1; idx = ++_j) {
      ...
      dot = this.svg.selectAll("circle.serie-" + idx).data(serie.data).enter().append('circle').attr('class', "serie-" + idx).attr('cx', function(d) {
        return x(_this._x(d));
      }).attr('cy', function(d) {
        return y(_this._y(d));
      }).attr('r', 5).attr('fill', serie.color).attr('stroke', 'none').attr('stroke-width', 3).style('cursor', 'pointer');
  ...
  Veasy.prototype.drawPie = function(series, opt) {
    ...
    return series.forEach(function(serie, idx) {
      var arc, color, g, pie;
      color = opt.color != null ? d3.scale.ordinal().range(opt.color) : d3.scale.category10();
